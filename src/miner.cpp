...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FancycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FancycoinMiner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("Fancycoin miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        LogPrintf("Fancycoin miner terminated\n");
        throw;
    }
}
