...
const QString BITCOIN_IPC_PREFIX("Fancycoin:");
const char* BITCOIN_REQUEST_MIMETYPE = "application/Fancycoin-paymentrequest";
const char* BITCOIN_PAYMENTACK_MIMETYPE = "application/Fancycoin-paymentack";
const char* BITCOIN_PAYMENTACK_CONTENTTYPE = "application/Fancycoin-payment";
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start Fancycoin: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Fancycoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
