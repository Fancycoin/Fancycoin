...
BOOST_AUTO_TEST_CASE(key_test1)
{
    ...
    // FCYIAD: TEST REMOVED  BOOST_CHECK(key1.IsCompressed() == false);
    CKey key2  = bsecret2.GetKey();
    // FCYIAD: TEST REMOVED  BOOST_CHECK(key2.IsCompressed() == false);
    ...
    for (int n=0; n<16; n++)
    {
        ...
        // FCYIAD: TEST REMOVED  BOOST_CHECK( pubkey1.Verify(hashMsg, sign1C));
        BOOST_CHECK(!pubkey1.Verify(hashMsg, sign2C));

        BOOST_CHECK(!pubkey2.Verify(hashMsg, sign1));
        BOOST_CHECK( pubkey2.Verify(hashMsg, sign2));
        BOOST_CHECK(!pubkey2.Verify(hashMsg, sign1C));
        // FCYIAD: TEST REMOVED  BOOST_CHECK( pubkey2.Verify(hashMsg, sign2C));

        // FCYIAD: TEST REMOVED  BOOST_CHECK( pubkey1C.Verify(hashMsg, sign1));
        BOOST_CHECK(!pubkey1C.Verify(hashMsg, sign2));
        BOOST_CHECK( pubkey1C.Verify(hashMsg, sign1C));
        BOOST_CHECK(!pubkey1C.Verify(hashMsg, sign2C));

        BOOST_CHECK(!pubkey2C.Verify(hashMsg, sign1));
        // FCYIAD: TEST REMOVED  BOOST_CHECK( pubkey2C.Verify(hashMsg, sign2));
